...
function NewsCard({ article, index, handleToggleSave, isSaved, keyword, savedArticles }) {

// const getOneArticle = () => {
//  return savedArticles.map((a) => {
//   return a.keyword.join("");
//   })
// };


// const handleTravelToUrl = (evt) => {
//   evt.stopPropagation();
...
function formatDate(dateString) {
  ...
  {savedNewsPage && article.keyword && (
    <p className="newscard__keyword">{article.keyword}</p>
            ...
            <button onMouseEnter={() => console.log("hovering")} onClick={() => handleToggleSave(article, keyword)} type="button" className={savedNewsPage ? "newscard__trashcan" : isSaved ? "newscard__bookmark_clicked" : "newscard__bookmark"}></button>
